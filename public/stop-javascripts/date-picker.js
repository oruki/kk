...
var DatePicker = {
  ...
  days: '日,月,火,水,木,金,土'.split(','),

  /* Method declarations */

  toggleDatePicker: function (id,showSuffixes,showWDay) {
    var calendar = this.findCalendarElement(id);
    showSuffixesInLink = showSuffixes ? true : false;
    showDayOfWeek = showWDay ? true : false;
  ...
  setDate: function (id, ansiDate) {
    ...
    var d_day_of_week = this.days[this.dateFromAnsiDate(ansiDate).getDay()]
    var linkDate;
    document.getElementById(id).value = ansiDate;
    if (showSuffixesInLink == true) {
      linkDate = d_year + '年' + d_mon + '月' + d_day + '日'
    } else {
      linkDate = d_year + '/' + d_mon + '/' +  d_day
    }
    if (showDayOfWeek == true) {
      linkDate += '(' + d_day_of_week + ')'
    }
    this.findLinkElement(id).innerHTML = linkDate
  },
  ...
  writeCalendar: function (id) {
    ...
    o +=
      ...
      '<th colspan="3">' + this.writeYear(id,  date.getFullYear()) + '年' + '</th>' +
    ...
    o +=
      ...
      '<th colspan="5">' + this.writeMonth(id, date.getMonth()) + '月 ' + date.getDate() + '日' + '</th>' +
    ...
    if (this.showCancelLink) {
      o += '<tfoot><tr><td colspan="7"><div class="cancel_butt"><a href="#" onclick="DatePicker.cancel(\''+id+'\')">[x] キャンセル</a></div></td></tr></tfoot>';
    }
